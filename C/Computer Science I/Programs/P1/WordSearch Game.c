...
int main() {
    ...
    char ranGrid;		//Boolean variable for whether the grid will be randomized or chosen
    ...
    while(numCases < min_numCases || numCases > max_numCases) {
        if (numCases < min_numCases)
            puts("Too few cases. Try again.");
        if (numCases > max_numCases)
            puts("Too many cases. Try again.");
        scanf("%d",&numCases);
    }

    printf("Would you like to create your own grid?\n\n Type \"y\" for yes or \"n\" for no.\n);
    scanf("%c",&ranGrid);
